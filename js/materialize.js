...
;(function ($) {
    $(document).ready(function() {
        ...
        $(document).on('click.card', 'click.dcard', '.card', '.dcard', function (e) {     if ($(this).find('> .card-reveal').length) {         if ($(e.target).is($('.card-reveal .card-title')) || $(e.target).is($('.card-reveal .card-title i'))) { 
            ...
            $(this).find('.card-reveal').css({ display: 'block'})
                .velocity("stop", false).velocity({translateY: '-100%'},
                {duration: 300, queue: false, easing: 'easeInOutQuad'}); 
        } 
        }  

        if ($(this).find('> .dcard-reveal').length) {         if ($(e.target).is($('.dcard-reveal .dcard-title')) || $(e.target).is($('.dcard-reveal .dcard-title i'))) {                         $(this).find('.dcard-reveal').velocity(                 {translateY: 0}, {                     duration: 225,                     queue: false,                     easing: 'easeInOutQuad',                     complete: function() { $(this).css({ display: 'none'}); }                 }             );         }         else if ($(e.target).is($('.dcard .activator')) ||             $(e.target).is($('.dcard .activator i')) ) {             $(e.target).closest('.dcard').css('overflow', 'hidden');             $(this).find('.dcard-reveal').css({ display: 'block'}).velocity("stop", false).velocity({translateY: '-100%'}, {duration: 300, queue: false, easing: 'easeInOutQuad'});         }     }  });
